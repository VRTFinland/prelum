...
LOGO_KEY = "assets/prelum-logo-concept.svg"
        ...
        if _LIST_ITEM.match(line) is not None:
            ...
            while index < len(lines) and (item := _LIST_ITEM.match(lines[index])) is not None:
                ...
                while index < len(lines) and lines[index].strip() and lines[index][0].isspace():
                    ...
                    if _LIST_ITEM.match(continuation) is not None:
                        raise _refuse(lines[index], continuation_number, "nested list")
                    if _NUMBERED_ITEM.match(continuation) is not None:
                        raise _refuse(lines[index], continuation_number, "numbered list")
                    for prefix, construct in _REFUSED:
                        if continuation.startswith(prefix):
                            raise _refuse(lines[index], continuation_number, construct)
                    if (
                        _HEADING.match(continuation) is not None
                        or _FENCE.match(continuation) is not None
                        or continuation.startswith("|")
                    ):
                        raise _refuse(lines[index], continuation_number, "indented block")
                    item_lines.append(continuation)
                    index += 1
                items.append(parse_inline(" ".join(item_lines)))
...
def nav_pages(config_path: Path) -> list[tuple[str | None, str, str]]:
    """The published pages, in navigation order, as (section, title, path) triples.

    Reading the order from `zensical.toml` rather than restating it keeps the manual's contents from
...
def _resolve_span(span: Span, page: str, pages: set[str], site_url: str) -> Span:
    ...
    if not target:
        return span
    # PurePosixPath joins but does not normalise, so a link into a sibling directory keeps its
    # '..' segment and matches nothing in the page set. MkDocs resolves it, and so must this.
    resolved = posixpath.normpath(str(PurePosixPath(page).parent / target))
...
def build_manual_data(root: Path) -> dict[str, Any]:
    ...
    config = tomllib.loads(config_path.read_text(encoding="utf-8"))["project"]
    docs = root / "docs"
    pages = [entry for entry in nav_pages(config_path) if entry[2] not in EXCLUDED_PAGES]
