...
_PROXY_VARIABLES = tuple(
    variable for name in ("http_proxy", "https_proxy", "all_proxy", "no_proxy") for variable in (name, name.upper())
)
...
def _encode_utf8(text: str, subject: str, *, key: str | None = None) -> bytes:
    """
    ...
    :param key: The files key the text belongs to, published in the error's context; None for source.
    """
    try:
        return text.encode("utf-8")
    except UnicodeEncodeError as exc:
        raise InvalidFileDataError(
            f"The {subject} contains characters that cannot be encoded as UTF-8",
            context={} if key is None else {"key": key},
        ) from exc
...
class TypstRenderer:
    ...
    async def render(self, job: RenderJob) -> RenderResult:
        ...
        if len(source_bytes) > self.settings.max_template_source_bytes:
            raise TemplateSourceTooLargeError(
                ...
                limit=self.settings.max_template_source_bytes,
            )
    ...
    def _write_inline_files(
        ...
        for relative_key, file_entry in files.items():
            ...
            if not dest.is_relative_to(resolved_root):
                raise InvalidFileDataError(
                    f"File path '{for_message(relative_key)}' escapes project root",
            ...
            try:
                ...
                raise InvalidFileDataError(
                    f"File path '{for_message(relative_key)}' cannot be written",
                    context={"key": relative_key},
                ) from exc
    ...
    def _decode_inline_file(self, key: str, entry: RenderFile) -> bytes:
        ...
        if entry.encoding == "base64":
            ...
            if decoded_size > limit:
                raise TemplateFileTooLargeError(key=key, size=decoded_size, limit=limit)
            try:
                return base64.b64decode(entry.content, validate=True)
            except (binascii.Error, ValueError) as exc:
                raise InvalidFileDataError(
                    f"Invalid base64 content for file '{for_message(key)}'",
                    context={"key": key},
                ) from exc

        content_bytes = _encode_utf8(entry.content, f"content of file '{for_message(key)}'", key=key)
        if len(content_bytes) > limit:
            raise TemplateFileTooLargeError(key=key, size=len(content_bytes), limit=limit)
    ...
    async def _run_typst(
        ...
        *,
        ...
        try:
            ...
            proc.kill()
            _ = await proc.wait()
            raise RenderTimeoutError(timeout_secs=self.settings.render_timeout_secs) from exc
        except asyncio.CancelledError:
            proc.kill()
            _ = await proc.wait()
            raise

        # proc.returncode is int | None; wait() hands back the settled code, so the sign test below
        # needs no narrowing.
        if returncode != 0:
            # A negative return code is not an exit status: a signal killed the process, and which
            # signal decides whose fault it is. Exactly one signal means "this template wanted more
            # memory than it may have", and which one depends on whether a bound is configured.
            # Bounded by RLIMIT_DATA, the allocation fails inside Typst and Rust aborts, so it is
            # SIGABRT — and a SIGKILL then came from outside the render: the container's own memory
            # limit, an eviction, a supervisor, usually meaning the deployment is under-provisioned
            # for its configured concurrency, which has to stay visible. Not always, though: the
            # bound covers the heap and anonymous mappings, so caller-supplied fonts that Typst
            # mmaps and scratch space on a tmpfs sit outside it, and on kernels before 4.7 so does
            # anonymous mmap altogether. A caller can therefore still drive a container-level kill
            # here. That is accepted: no classification can tell such a kill from an
            # under-provisioned pod, and a 5xx that is occasionally caller-driven is the safer of
            # the two mistakes. Unbounded, the kernel's OOM killer
            ...
            error: AppError
            if killed_by == out_of_memory:
                error = InlineTemplateError("Template rendering was killed for exhausting available memory")
    ...
    def _json_to_typst(self, value: JSONValue) -> str:
        ...
        # The walk carries no position. Building a path on every node would cost every successful
        # render for the benefit of the failing one, so each frame adds its own segment only while a
        # StringTooLargeError unwinds through it.
        if isinstance(value, list):
            items: list[str] = []
            for index, item in enumerate(value):
                try:
                    items.append(self._json_to_typst(item))
                except StringTooLargeError as exc:
                    exc.path.insert(0, index)
                    raise
        ...
        for key, val in value.items():
            key_text = str(key)
            try:
                # An oversized key is its own last path segment, which reads exactly like the path
                # to the value stored under it. Only context.subject tells the two apart, so the
                # key is escaped on its own rather than inside the expression the value builds.
                escaped_key = self._escape_string(key_text, subject="key")
                parts.append(f'"{escaped_key}": {self._json_to_typst(val)}')
            except StringTooLargeError as exc:
                exc.path.insert(0, for_message(key_text))
                raise
        return f"({', '.join(parts)})" if parts else "(:)"

    def _escape_string(self, input_str: str, *, subject: Literal["key", "value"] = "value") -> str:
        """
        Escape a string for inclusion in Typst code, per the _TYPST_ESCAPES table.

        :param subject: Whether the string is an object key or a value, published so a caller can
            tell an oversized key from an oversized value: both carry the same path.
        ...
        if string_bytes > self.settings.max_string_bytes:
            raise StringTooLargeError(
                size=string_bytes,
                limit=self.settings.max_string_bytes,
                subject=subject,
            )
