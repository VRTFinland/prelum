...
from app.render.templates import PROJECT_ROOT_RESERVE
...
@pytest.mark.parametrize("key", [".", "./", "./main.typ", INLINE_TEMPLATE_FILENAME])
...
@pytest.mark.parametrize("key", ["main.typ", "main.typ/foo.typ"])
def test_render_request_accepts_keys_that_no_longer_collide_with_the_entry_point(key: str):
    """The inline source is written to a name no key can spell, so nothing is reserved."""
    request = RenderRequest(source='#text("hi")', files={key: RenderFile(encoding="text", content="x")})

    assert key in request.files
...
def test_temp_root_defaults_to_the_short_controlled_path():
    assert Settings().temp_root == Path("/tmp")  # noqa: S108 - asserts the documented default


def test_temp_root_must_leave_room_for_the_render_project_root(tmp_path: Path):
    """
    The published key-length bound is derived from the reserve, so a long root would void it.

    Rejecting at construction rather than per render keeps the contract an operator breaks visible
    at startup instead of as a 503 on the first request.
    """
    # Nested rather than one long name: a single component is capped at NAME_MAX well before the
    # whole path reaches the reserve.
    too_long = tmp_path.joinpath(*["d" * 60] * 4)
    too_long.mkdir(parents=True)
    assert len(str(too_long)) > PROJECT_ROOT_RESERVE

    with pytest.raises(ValidationError, match="temp root"):
        _ = Settings(temp_root=too_long)


def test_temp_root_must_be_an_existing_absolute_directory(tmp_path: Path):
    with pytest.raises(ValidationError):
        _ = Settings(temp_root=Path("relative/path"))
    with pytest.raises(ValidationError):
        _ = Settings(temp_root=tmp_path / "missing")


def test_render_memory_limit_is_off_by_default():
    """prlimit is Linux-only and development is not, so the wrapper stays off until asked for."""
    assert Settings().max_render_memory_bytes is None


def test_render_memory_limit_refuses_a_value_below_the_measured_floor(monkeypatch: pytest.MonkeyPatch):
    """
    A realistic 300-section A4 document aborts under RLIMIT_DATA at 64 MiB and renders at 128 MiB.

    Below the floor a legitimate render dies the same way a runaway one does, so the operator gets
    a stream of 422s and no signal that the limit is what caused them.
    """
    monkeypatch.setattr(constants, "PRLIMIT_PATH", Path("/bin/sh"))

    with pytest.raises(ValidationError):
        _ = Settings(max_render_memory_bytes=64 * 1024 * 1024)


def test_render_memory_limit_requires_the_wrapper_to_exist(monkeypatch: pytest.MonkeyPatch):
    """
    Every way prlimit itself fails exits positive, which the renderer reads as a caller fault.

    A missing wrapper would therefore answer 422 on every request instead of failing the
    deployment, so its absence has to stop the service from constructing at all.
    """
    monkeypatch.setattr(constants, "PRLIMIT_PATH", Path("/nonexistent/prlimit"))

    with pytest.raises(ValidationError, match="prlimit"):
        _ = Settings(max_render_memory_bytes=1024 * 1024 * 1024)


def test_render_memory_limit_is_accepted_when_the_wrapper_is_present(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(constants, "PRLIMIT_PATH", Path("/bin/sh"))

    assert Settings(max_render_memory_bytes=1024 * 1024 * 1024).max_render_memory_bytes == 1073741824


def test_temp_root_must_be_writable(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    """
    A read-only root filesystem is a documented Kubernetes hardening, and /tmp is then a volume.

    Omit that volume and every render fails in TemporaryDirectory with EROFS — a 503 per request
    from a pod that answers /health and joins its Service. The point of validating here is that the
    misconfiguration stops the deployment instead.

    The unwritable root is simulated rather than built with chmod, because the container CI runs in
    is root, and root bypasses permission bits. It does not bypass a read-only mount, which is the
    case this guards; EROFS reaches the validator as the OSError raised here.
    """
    real_temporary_directory = tempfile.TemporaryDirectory

    def refuse(*args: object, **kwargs: object) -> object:
        if kwargs.get("dir") == tmp_path:
            raise OSError(errno.EROFS, "Read-only file system")
        return real_temporary_directory(*args, **kwargs)  # pyright: ignore[reportCallIssue]

    monkeypatch.setattr(tempfile, "TemporaryDirectory", refuse)

    with pytest.raises(ValidationError, match="writable"):
        _ = Settings(temp_root=tmp_path)


def test_the_default_temp_root_is_validated_too():
    """
    Pydantic skips validators for defaults unless the field asks for them.

    The published image sets no PRELUM_TEMP_ROOT, so the default is exactly the configuration where
    an unwritable root matters most — and the one that would never have been checked. Asserted on
    the field rather than by constructing one, because the default is bound at class definition.
    """
    assert Settings.model_fields["temp_root"].validate_default is True


def test_an_empty_render_memory_limit_reads_as_unset(monkeypatch: pytest.MonkeyPatch):
    """
    The image bakes the limit into ENV, and neither Docker nor Kubernetes can unset an image ENV.

    Both documentation and the validator tell operators to leave it unset where prlimit cannot
    work, so an empty value has to be the way to say that — otherwise the advice is unfollowable
    and the override fails with a message about parsing integers.
    """
