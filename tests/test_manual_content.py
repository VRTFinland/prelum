...
def test_wrapped_bullet_lines_stay_in_their_list_item():
    blocks = parse_markdown("- First item wraps onto\n  a second `line`;\n- Second item\n  also wraps.\n")
    ...
    assert blocks == [
        {
            ...
            "items": [
                [
                    {"kind": "text", "text": "First item wraps onto a second "},
                    {"kind": "code", "text": "line"},
                    {"kind": "text", "text": ";"},
                ],
                [{"kind": "text", "text": "Second item also wraps."}],
            ],
        }
    ]


def test_nested_bullet_list_is_refused_instead_of_flattened():
    with pytest.raises(UnsupportedMarkdownError, match="nested list"):
        _ = parse_markdown("- Parent\n  - Child\n")
...
def test_every_published_documentation_page_converts():
    ...
    pages = [page for _, _, page in nav_pages(ROOT / "zensical.toml") if page not in EXCLUDED_PAGES]
...
def test_manual_data_carries_every_published_page_in_navigation_order():
    ...
    expected = [title for _, title, page in nav_pages(ROOT / "zensical.toml") if page not in EXCLUDED_PAGES]
...
def test_a_link_into_a_sibling_directory_becomes_a_chapter_reference():
    """
    MkDocs resolves '../api/render.md' against the page that carries it; joining leaves the '..'.

    An unnormalised 'operations/../api/render.md' matches nothing in the page set, so three real
    cross-references in the published docs turned into web links inside the PDF — where the whole
    point is that an internal reference stays internal.
    """
    blocks = resolve_links(
        parse_markdown("See [Render request](../api/render.md#add-project-files).\n"),
        page="operations/fonts-and-packages.md",
        pages={"api/render.md"},
        site_url="https://example.test/prelum/",
    )
    ...
    assert blocks[0]["spans"][1] == {
        "kind": "ref",
        "text": "Render request",
        "target": chapter_id("api/render.md"),
    }
