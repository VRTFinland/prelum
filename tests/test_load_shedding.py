...
async def test_shed_response_carries_retry_after(shedding_app):
    ...
    # The header and the published field must never tell the caller two different things.
    assert response.json()["context"] == {"retry_after": int(response.headers["Retry-After"])}
